...
import { type FlatfileRecord, bulkRecordHook } from '@flatfile/plugin-record-hook'
...
import { addGuidances, addGuides, setDefaultPage, uploadFiles } from '../utils'
...
export default function flatfileEventListener(listener: FlatfileListener) {
  listener.use(
    configureSpace(
      ...
      async (event: FlatfileEvent, workbookIds: string[], tick: TickFunction) => {
        ...
        // Upload demo files
        const industryFilesFolderPath = path.join(__dirname, `../files/${industry}`)
        const files = fs.readdirSync(industryFilesFolderPath).filter((file) => file.endsWith('.csv'))
        const fileUrls = files.map(
          (file) => `https://github.com/FlatFilers/demo-agent/raw/refs/heads/main/src/files/${industry}/${file}`,
        )
        await uploadFiles({ spaceId, environmentId }, fileUrls)
      },
    ),
  )
  ...
  listener.use(
    automap({
      accuracy: 'confident',
      defaultTargetSheet: 'Property Types',
